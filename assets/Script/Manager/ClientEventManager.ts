...
export class ClientEventManager {
    ...
    public static on (eventName: string, handler: Function, target: any) {
        ...
        var handlerList: Array<any> = ClientEventManager._handlers[eventName];
        if (!handlerList) {
            handlerList = [];
            ClientEventManager._handlers[eventName] = handlerList;
        }
    ...
    public static off (eventName: string, handler: Function, target: any) {
        var handlerList = ClientEventManager._handlers[eventName];
    ...
    public static dispatchEvent (eventName: string, ...args: any) {
        ...
        var args1 = []; 
        var i;
        for (i = /* 1 */0; i < arguments.length; i++) {
